...
##trying something
sensor_dgs = (df_swillup, df_butler,df_cecilville,df_happy_camp_cc,df_sawyer,df_forks,df_kdnr_out,df_somesbar,df_sandybar)
 


'''
for df in sensor_dgs:
    df_name = df['location_label']
    df.to_csv('/Users/erikschubert/Desktop/AirQualityResearch/code/'+ str(df_name) +'21.csv')
...
def Tracking_Events_Greater_Than_Given_Perameters(concentration_param,Event_level,sensor_list_gf):
   ...
   for k,df in enumerate(sensor_list_gf):
      ...
      for i in df_resampled.index:
         ...
         df_resampled.loc[i,'event_count'] = (df_resampled['corrected'].loc[a:j]>concentration_param).sum()
     ...
     # my_dict['name']='Nick'
      ...
      for i in consolidated_events.index:
         ...
         #lables = 'Event {i +1} , Duration ' + str(consolidated_events['days'].loc[i]
